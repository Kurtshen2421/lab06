...
int scatterVectorReceiver(int senderRank, parallelVector *vecP){
	...
	int tag = 0;
	MPI_Status status;
	int flag = MPI_Recv(vecP->dataLocal, vecP->nLocal, MPI_FLOAT, senderRank, tag, MPI_COMM_WORLD, &status);
	// ================END OF YOUR CODE==================================
	...
	if(rank == rankToCollectNorm){
                for(i=0; i<n_procs; ++i){ // for each other rank, send it the totalNorm
                        if(i != rankToCollectNorm){
                                localSum = MPI_Send(&localSum, 1, MPI_FLOAT, i, tag, MPI_COMM_WORLD);
                        }
                }
        }
